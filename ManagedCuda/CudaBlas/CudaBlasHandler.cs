...
namespace ManagedCuda.CudaBlas
{
	...
	public class CudaBlas
	{
		...
		public void MatinvBatched(int n, CudaDeviceVariable<float>[] A, int lda,
														   CudaDeviceVariable<float>[] Ainv, int lda_inv, CudaDeviceVariable<int> INFO, int batchSize)
		{
			if (A.Length != batchSize || A.Length != Ainv.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");

			CUdeviceptr[] arrA, arrAinv;
			arrA = new CUdeviceptr[A.Length];
			arrAinv = new CUdeviceptr[Ainv.Length];

			for (int i = 0; i < batchSize; i++)
			{
				arrA[i] = A[i].DevicePointer;
				arrAinv[i] = Ainv[i].DevicePointer;
			}

			_status = CudaBlasNativeMethods.cublasSmatinvBatched(_blasHandle, n, arrA, lda, arrAinv, lda_inv, INFO.DevicePointer, batchSize);
		...
		/// <param name="INFO"></param>
		/// <param name="batchSize"></param>
		public void MatinvBatched(int n, CudaDeviceVariable<double>[] A, int lda,
														   CudaDeviceVariable<double>[] Ainv, int lda_inv, CudaDeviceVariable<int> INFO, int batchSize)
		{
			if (A.Length != batchSize || A.Length != Ainv.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasDmatinvBatched(_blasHandle, n, arrA, lda, arrAinv, lda_inv, INFO.DevicePointer, batchSize);
		...
		/// <param name="INFO"></param>
		/// <param name="batchSize"></param>
		public void MatinvBatched(int n, CudaDeviceVariable<cuFloatComplex>[] A, int lda,
														   CudaDeviceVariable<cuFloatComplex>[] Ainv, int lda_inv, CudaDeviceVariable<int> INFO, int batchSize)
		{
			if (A.Length != batchSize || A.Length != Ainv.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasCmatinvBatched(_blasHandle, n, arrA, lda, arrAinv, lda_inv, INFO.DevicePointer, batchSize);
		...
		/// <param name="INFO"></param>
		/// <param name="batchSize"></param>
		public void MatinvBatched(int n, CudaDeviceVariable<cuDoubleComplex>[] A, int lda,
														   CudaDeviceVariable<cuDoubleComplex>[] Ainv, int lda_inv, CudaDeviceVariable<int> INFO, int batchSize)
		{
			if (A.Length != batchSize || A.Length != Ainv.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasZmatinvBatched(_blasHandle, n, arrA, lda, arrAinv, lda_inv, INFO.DevicePointer, batchSize);
		...
		public void SgemmBatched(Operation transa, Operation transb, int m, int n, int k, CudaDeviceVariable<float> alpha,
								   CudaDeviceVariable<float>[] Aarray, int lda, CudaDeviceVariable<float>[] Barray, int ldb,
								   CudaDeviceVariable<float> beta, CudaDeviceVariable<float>[] Carray, int ldc, int batchCount)
		{
			if (Aarray.Length != Barray.Length || Barray.Length != Carray.Length || Carray.Length != batchCount)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");

			CUdeviceptr[] arrA, arrB, arrC;
			arrA = new CUdeviceptr[Aarray.Length];
			arrB = new CUdeviceptr[Barray.Length];
			arrC = new CUdeviceptr[Carray.Length];

			for (int i = 0; i < batchCount; i++)
			{
				arrA[i] = Aarray[i].DevicePointer;
				arrB[i] = Barray[i].DevicePointer;
				arrC[i] = Carray[i].DevicePointer;
			}

			_status = CudaBlasNativeMethods.cublasSgemmBatched(_blasHandle, transa, transb, m, n, k, alpha.DevicePointer, arrA, lda, arrB, ldb, beta.DevicePointer, arrC, ldc, batchCount);
		...
		/// <param name="ldc">leading dimension of two-dimensional array used to store each matrix C[i].</param>
		/// <param name="batchCount">number of pointers contained in A, B and C.</param>
		public void DgemmBatched(Operation transa, Operation transb, int m, int n, int k, CudaDeviceVariable<double> alpha,
								   CudaDeviceVariable<double>[] Aarray, int lda, CudaDeviceVariable<double>[] Barray, int ldb, CudaDeviceVariable<double> beta,
								   CudaDeviceVariable<double>[] Carray, int ldc, int batchCount)
		{
			if (Aarray.Length != Barray.Length || Barray.Length != Carray.Length || Carray.Length != batchCount)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
			...
			_status = CudaBlasNativeMethods.cublasDgemmBatched(_blasHandle, transa, transb, m, n, k, alpha.DevicePointer, arrA, lda, arrB, ldb, beta.DevicePointer, arrC, ldc, batchCount);
		...
		/// <param name="ldc">leading dimension of two-dimensional array used to store each matrix C[i].</param>
		/// <param name="batchCount">number of pointers contained in A, B and C.</param>
		public void CgemmBatched(Operation transa, Operation transb, int m, int n, int k, CudaDeviceVariable<cuFloatComplex> alpha, 
			CudaDeviceVariable<cuFloatComplex>[] Aarray, int lda, CudaDeviceVariable<cuFloatComplex>[] Barray, int ldb,
			CudaDeviceVariable<cuFloatComplex> beta, CudaDeviceVariable<cuFloatComplex>[] Carray, int ldc, int batchCount)
		{
			if (Aarray.Length != Barray.Length || Barray.Length != Carray.Length || Carray.Length != batchCount)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
			...
			_status = CudaBlasNativeMethods.cublasCgemmBatched(_blasHandle, transa, transb, m, n, k, alpha.DevicePointer, arrA, lda, arrB, ldb, beta.DevicePointer, arrC, ldc, batchCount);
		...
		/// <param name="ldc">leading dimension of two-dimensional array used to store each matrix C[i].</param>
		/// <param name="batchCount">number of pointers contained in A, B and C.</param>
		public void ZgemmBatched(Operation transa, Operation transb, int m, int n, int k, CudaDeviceVariable<cuDoubleComplex> alpha,
			CudaDeviceVariable<cuDoubleComplex>[] Aarray, int lda, CudaDeviceVariable<cuDoubleComplex>[] Barray, int ldb,
			CudaDeviceVariable<cuDoubleComplex> beta, CudaDeviceVariable<cuDoubleComplex>[] Carray, int ldc, int batchCount)
		{
			if (Aarray.Length != Barray.Length || Barray.Length != Carray.Length || Carray.Length != batchCount)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
			...
			_status = CudaBlasNativeMethods.cublasZgemmBatched(_blasHandle, transa, transb, m, n, k, alpha.DevicePointer, arrA, lda, arrB, ldb, beta.DevicePointer, arrC, ldc, batchCount);
		...
		/// <param name="ldc">leading dimension of two-dimensional array used to store each matrix C[i].</param>
		/// <param name="batchCount">number of pointers contained in A, B and C.</param>
		public void SgemmBatched(Operation transa, Operation transb, int m, int n, int k, ref float alpha, CudaDeviceVariable<float>[] Aarray,
			int lda, CudaDeviceVariable<float>[] Barray, int ldb, ref float beta, CudaDeviceVariable<float>[] Carray, int ldc, int batchCount)
		{
			if (Aarray.Length != Barray.Length || Barray.Length != Carray.Length || Carray.Length != batchCount)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
			...
			_status = CudaBlasNativeMethods.cublasSgemmBatched(_blasHandle, transa, transb, m, n, k, ref alpha, arrA, lda, arrB, ldb, ref beta, arrC, ldc, batchCount);
			Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cublasSgemmBatched", _status));
			if (_status != CublasStatus.Success) throw new CudaBlasException(_status);
		}
		...
		/// <param name="ldc">leading dimension of two-dimensional array used to store each matrix C[i].</param>
		/// <param name="batchCount">number of pointers contained in A, B and C.</param>
		public void DgemmBatched(Operation transa, Operation transb, int m, int n, int k, ref double alpha, 
			CudaDeviceVariable<double>[] Aarray, int lda, CudaDeviceVariable<double>[] Barray, int ldb, ref double beta,
			CudaDeviceVariable<double>[] Carray, int ldc, int batchCount)
		{
			if (Aarray.Length != Barray.Length || Barray.Length != Carray.Length || Carray.Length != batchCount)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
			...
			_status = CudaBlasNativeMethods.cublasDgemmBatched(_blasHandle, transa, transb, m, n, k, ref alpha, arrA, lda, arrB, ldb, ref beta, arrC, ldc, batchCount);
			Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cublasDgemmBatched", _status));
			if (_status != CublasStatus.Success) throw new CudaBlasException(_status);
		}
		...
		/// <param name="ldc">leading dimension of two-dimensional array used to store each matrix C[i].</param>
		/// <param name="batchCount">number of pointers contained in A, B and C.</param>
		public void CgemmBatched(Operation transa, Operation transb, int m, int n, int k, ref cuFloatComplex alpha,
			CudaDeviceVariable<cuFloatComplex>[] Aarray, int lda, CudaDeviceVariable<cuFloatComplex>[] Barray, int ldb,
			ref cuFloatComplex beta, CudaDeviceVariable<cuFloatComplex>[] Carray, int ldc, int batchCount)
		{
			if (Aarray.Length != Barray.Length || Barray.Length != Carray.Length || Carray.Length != batchCount)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
			...
			_status = CudaBlasNativeMethods.cublasCgemmBatched(_blasHandle, transa, transb, m, n, k, ref alpha, arrA, lda, arrB, ldb, ref beta, arrC, ldc, batchCount);
			Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cublasCgemmBatched", _status));
			if (_status != CublasStatus.Success) throw new CudaBlasException(_status);
		}
		...
		/// <param name="ldc">leading dimension of two-dimensional array used to store each matrix C[i].</param>
		/// <param name="batchCount">number of pointers contained in A, B and C.</param>
		public void ZgemmBatched(Operation transa, Operation transb, int m, int n, int k, ref cuDoubleComplex alpha,
			CudaDeviceVariable<cuDoubleComplex>[] Aarray, int lda, CudaDeviceVariable<cuDoubleComplex>[] Barray, int ldb,
			ref cuDoubleComplex beta, CudaDeviceVariable<cuDoubleComplex>[] Carray, int ldc, int batchCount)
		{
			if (Aarray.Length != Barray.Length || Barray.Length != Carray.Length || Carray.Length != batchCount)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
			...
			_status = CudaBlasNativeMethods.cublasZgemmBatched(_blasHandle, transa, transb, m, n, k, ref alpha, arrA, lda, arrB, ldb, ref beta, arrC, ldc, batchCount);
			Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cublasZgemmBatched", _status));
			if (_status != CublasStatus.Success) throw new CudaBlasException(_status);
		}
		...
		public void SgetrfBatched(int n, CudaDeviceVariable<float>[] A, int lda, CudaDeviceVariable<int> P,
			CudaDeviceVariable<int> INFO, int batchSize)
		{
			...
			_status = CudaBlasNativeMethods.cublasSgetrfBatched(_blasHandle, n, arrA, lda, P.DevicePointer, INFO.DevicePointer, batchSize);
		...
		/// If info = i, aii is 0. The factorization has been completed, but U is exactly singular.</param>
		/// <param name="batchSize">number of pointers contained in A</param>
		public void DgetrfBatched(int n, CudaDeviceVariable<double>[] A, int lda, CudaDeviceVariable<int> P,
			CudaDeviceVariable<int> INFO, int batchSize)
		{
			if (A.Length != batchSize)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
			...
			_status = CudaBlasNativeMethods.cublasDgetrfBatched(_blasHandle, n, arrA, lda, P.DevicePointer, INFO.DevicePointer, batchSize);
		...
		/// If info = i, aii is 0. The factorization has been completed, but U is exactly singular.</param>
		/// <param name="batchSize">number of pointers contained in A</param>
		public void CgetrfBatched(int n, CudaDeviceVariable<cuFloatComplex>[] A, int lda,
			CudaDeviceVariable<int> P, CudaDeviceVariable<int> INFO, int batchSize)
		{
			...
			_status = CudaBlasNativeMethods.cublasCgetrfBatched(_blasHandle, n, arrA, lda, P.DevicePointer, INFO.DevicePointer, batchSize);
		...
		/// If info = i, aii is 0. The factorization has been completed, but U is exactly singular.</param>
		/// <param name="batchSize">number of pointers contained in A</param>
		public void ZgetrfBatched(int n, CudaDeviceVariable<cuDoubleComplex>[] A, int lda,
			CudaDeviceVariable<int> P, CudaDeviceVariable<int> INFO, int batchSize)
		{
			if (A.Length != batchSize)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
			...
			_status = CudaBlasNativeMethods.cublasZgetrfBatched(_blasHandle, n, arrA, lda, P.DevicePointer, INFO.DevicePointer, batchSize);
		...
		/// <param name="batchSize">number of pointers contained in A</param>
		public void GetriBatched(int n, CudaDeviceVariable<float>[] Aarray, int lda, CudaDeviceVariable<int> P,
			CudaDeviceVariable<float>[] Carray, int ldc, CudaDeviceVariable<int> INFO, int batchSize)
		{
			if (Aarray.Length != batchSize || Aarray.Length != Carray.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasSgetriBatched(_blasHandle, n, arrA, lda, P.DevicePointer, arrC, ldc, INFO.DevicePointer, batchSize);
		...
		/// If info = k, U(k,k) is 0. The U is exactly singular and the inversion failed.</param>
		/// <param name="batchSize">number of pointers contained in A</param>
		public void GetriBatched(int n, CudaDeviceVariable<double>[] Aarray, int lda, CudaDeviceVariable<int> P,
			CudaDeviceVariable<double>[] Carray, int ldc, CudaDeviceVariable<int> INFO, int batchSize)
		{
			if (Aarray.Length != batchSize || Aarray.Length != Carray.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasDgetriBatched(_blasHandle, n, arrA, lda, P.DevicePointer, arrC, ldc, INFO.DevicePointer, batchSize);
		...
		/// If info = k, U(k,k) is 0. The U is exactly singular and the inversion failed.</param>
		/// <param name="batchSize">number of pointers contained in A</param>
		public void GetriBatched(int n, CudaDeviceVariable<cuFloatComplex>[] Aarray, int lda, CudaDeviceVariable<int> P,
			CudaDeviceVariable<cuFloatComplex>[] Carray, int ldc, CudaDeviceVariable<int> INFO, int batchSize)
		{
			if (Aarray.Length != batchSize || Aarray.Length != Carray.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasCgetriBatched(_blasHandle, n, arrA, lda, P.DevicePointer, arrC, ldc, INFO.DevicePointer, batchSize);
		...
		/// If info = k, U(k,k) is 0. The U is exactly singular and the inversion failed.</param>
		/// <param name="batchSize">number of pointers contained in A</param>
		public void GetriBatched(int n, CudaDeviceVariable<cuDoubleComplex>[] Aarray, int lda, CudaDeviceVariable<int> P,
			CudaDeviceVariable<cuDoubleComplex>[] Carray, int ldc, CudaDeviceVariable<int> INFO, int batchSize)
		{
			if (Aarray.Length != batchSize || Aarray.Length != Carray.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasZgetriBatched(_blasHandle, n, arrA, lda, P.DevicePointer, arrC, ldc, INFO.DevicePointer, batchSize);
		...
		public void StrsmBatched(SideMode side, FillMode uplo, Operation trans, DiagType diag, int m, int n, CudaDeviceVariable<float> alpha,
			CudaDeviceVariable<float>[] A, int lda, CudaDeviceVariable<float>[] B, int ldb, int batchCount)
		{
			if (A.Length != batchCount || A.Length != B.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");

			CUdeviceptr[] arrA, arrB;
			arrA = new CUdeviceptr[A.Length];
			arrB = new CUdeviceptr[B.Length];

			for (int i = 0; i < batchCount; i++)
			{
				arrA[i] = A[i].DevicePointer;
				arrB[i] = B[i].DevicePointer;
			}

			_status = CudaBlasNativeMethods.cublasStrsmBatched(_blasHandle, side, uplo, trans, diag, m, n, alpha.DevicePointer,
				arrA, lda, arrB, ldb, batchCount);
		...
		/// <param name="ldb">leading dimension of two-dimensional array used to store matrix B[i].</param>
		/// <param name="batchCount"></param>
		public void DtrsmBatched(SideMode side, FillMode uplo, Operation trans, DiagType diag, int m, int n, CudaDeviceVariable<double> alpha,
			CudaDeviceVariable<double>[] A, int lda, CudaDeviceVariable<double>[] B, int ldb, int batchCount)
		{
			if (A.Length != batchCount || A.Length != B.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
		...
		/// <param name="ldb">leading dimension of two-dimensional array used to store matrix B[i].</param>
		/// <param name="batchCount"></param>
		public void CtrsmBatched(SideMode side, FillMode uplo, Operation trans, DiagType diag, int m, int n, CudaDeviceVariable<cuFloatComplex> alpha,
			CudaDeviceVariable<cuFloatComplex>[] A, int lda, CudaDeviceVariable<cuFloatComplex>[] B, int ldb, int batchCount)
		{
			if (A.Length != batchCount || A.Length != B.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
		...
		/// <param name="ldb">leading dimension of two-dimensional array used to store matrix B[i].</param>
		/// <param name="batchCount"></param>
		public void ZtrsmBatched(SideMode side, FillMode uplo, Operation trans, DiagType diag, int m, int n, CudaDeviceVariable<cuDoubleComplex> alpha,
			CudaDeviceVariable<cuDoubleComplex>[] A, int lda, CudaDeviceVariable<cuDoubleComplex>[] B, int ldb, int batchCount)
		{
			if (A.Length != batchCount || A.Length != B.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
		...
		/// <param name="ldb">leading dimension of two-dimensional array used to store matrix B[i].</param>
		/// <param name="batchCount"></param>
		public void StrsmBatched(SideMode side, FillMode uplo, Operation trans, DiagType diag, int m, int n, ref float alpha,
			CudaDeviceVariable<float>[] A, int lda, CudaDeviceVariable<float>[] B, int ldb, int batchCount)
		{
			if (A.Length != batchCount || A.Length != B.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
		...
		/// <param name="ldb">leading dimension of two-dimensional array used to store matrix B[i].</param>
		/// <param name="batchCount"></param>
		public void DtrsmBatched(SideMode side, FillMode uplo, Operation trans, DiagType diag, int m, int n, ref double alpha,
			CudaDeviceVariable<double>[] A, int lda, CudaDeviceVariable<double>[] B, int ldb, int batchCount)
		{
			if (A.Length != batchCount || A.Length != B.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
		...
		/// <param name="ldb">leading dimension of two-dimensional array used to store matrix B[i].</param>
		/// <param name="batchCount"></param>
		public void CtrsmBatched(SideMode side, FillMode uplo, Operation trans, DiagType diag, int m, int n, ref cuFloatComplex alpha,
			CudaDeviceVariable<cuFloatComplex>[] A, int lda, CudaDeviceVariable<cuFloatComplex>[] B, int ldb, int batchCount)
		{
			if (A.Length != batchCount || A.Length != B.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
		...
		/// <param name="ldb">leading dimension of two-dimensional array used to store matrix B[i].</param>
		/// <param name="batchCount"></param>
		public void ZtrsmBatched(SideMode side, FillMode uplo, Operation trans, DiagType diag, int m, int n, ref cuDoubleComplex alpha,
			CudaDeviceVariable<cuDoubleComplex>[] A, int lda, CudaDeviceVariable<cuDoubleComplex>[] B, int ldb, int batchCount)
		{
			if (A.Length != batchCount || A.Length != B.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchCount.");
		...
		public int GeqrfBatched(int m, int n, CudaDeviceVariable<float>[] Aarray, int lda, CudaDeviceVariable<float>[] TauArray)
		{
			int info = 0;
			CUdeviceptr[] _Aarray = new CUdeviceptr[Aarray.Length];
			CUdeviceptr[] _Tauarray = new CUdeviceptr[TauArray.Length];
			for (int i = 0; i < Aarray.Length; i++)
			{
				_Aarray[i] = Aarray[i].DevicePointer;
			}
			for (int i = 0; i < TauArray.Length; i++)
			{
				_Tauarray[i] = TauArray[i].DevicePointer;
			}
			_status = CudaBlasNativeMethods.cublasSgeqrfBatched(_blasHandle, m, n, _Aarray, lda, _Tauarray, ref info, _Aarray.Length);
		...
		/// <param name="TauArray">array of pointers to device vector, with each vector of dim. max(1,min(m,n)).</param>
		/// <returns>0, if the parameters passed to the function are valid, &lt;0, if the parameter in postion -value is invalid</returns>
		public int GeqrfBatched(int m, int n, CudaDeviceVariable<double>[] Aarray, int lda, CudaDeviceVariable<double>[] TauArray)
		{
			int info = 0;
			CUdeviceptr[] _Aarray = new CUdeviceptr[Aarray.Length];
			CUdeviceptr[] _Tauarray = new CUdeviceptr[TauArray.Length];
			...
			_status = CudaBlasNativeMethods.cublasDgeqrfBatched(_blasHandle, m, n, _Aarray, lda, _Tauarray, ref info, _Aarray.Length);
		...
		/// <param name="TauArray">array of pointers to device vector, with each vector of dim. max(1,min(m,n)).</param>
		/// <returns>0, if the parameters passed to the function are valid, &lt;0, if the parameter in postion -value is invalid</returns>
		public int GeqrfBatched(int m, int n, CudaDeviceVariable<cuFloatComplex>[] Aarray, int lda, CudaDeviceVariable<cuFloatComplex>[] TauArray)
		{
			int info = 0;
			CUdeviceptr[] _Aarray = new CUdeviceptr[Aarray.Length];
			CUdeviceptr[] _Tauarray = new CUdeviceptr[TauArray.Length];
			...
			_status = CudaBlasNativeMethods.cublasCgeqrfBatched(_blasHandle, m, n, _Aarray, lda, _Tauarray, ref info, _Aarray.Length);
		...
		/// <param name="TauArray">array of pointers to device vector, with each vector of dim. max(1,min(m,n)).</param>
		/// <returns>0, if the parameters passed to the function are valid, &lt;0, if the parameter in postion -value is invalid</returns>
		public int GeqrfBatched(int m, int n, CudaDeviceVariable<cuDoubleComplex>[] Aarray, int lda, CudaDeviceVariable<cuDoubleComplex>[] TauArray)
		{
			int info = 0;
			CUdeviceptr[] _Aarray = new CUdeviceptr[Aarray.Length];
			CUdeviceptr[] _Tauarray = new CUdeviceptr[TauArray.Length];
			...
			_status = CudaBlasNativeMethods.cublasZgeqrfBatched(_blasHandle, m, n, _Aarray, lda, _Tauarray, ref info, _Aarray.Length);
		...
		/// <returns>0, if the parameters passed to the function are valid, &lt;0, if the parameter in postion -value is invalid</returns>
		public int GelsBatched(Operation trans, int m, int n, int nrhs, CudaDeviceVariable<float>[] Aarray, int lda, CudaDeviceVariable<float>[] Carray, int ldc, CudaDeviceVariable<int> devInfoArray)
		{
			int info = 0;
			CUdeviceptr[] _Aarray = new CUdeviceptr[Aarray.Length];
			CUdeviceptr[] _Carray = new CUdeviceptr[Carray.Length];
			...
			{
				_Carray[i] = Carray[i].DevicePointer;
			}
			_status = CudaBlasNativeMethods.cublasSgelsBatched(_blasHandle, trans, m, n, nrhs, _Aarray, lda, _Carray, ldc, ref info, _devInfoArray, _Aarray.Length);
		...
		/// <param name="devInfoArray">null or optional array of integers of dimension batchsize.</param>
		/// <returns>0, if the parameters passed to the function are valid, &lt;0, if the parameter in postion -value is invalid</returns>
		public int GelsBatched(Operation trans, int m, int n, int nrhs, CudaDeviceVariable<double>[] Aarray, int lda, CudaDeviceVariable<double>[] Carray, int ldc, CudaDeviceVariable<int> devInfoArray)
		{
			int info = 0;
			CUdeviceptr[] _Aarray = new CUdeviceptr[Aarray.Length];
			CUdeviceptr[] _Carray = new CUdeviceptr[Carray.Length];
			...
			_status = CudaBlasNativeMethods.cublasDgelsBatched(_blasHandle, trans, m, n, nrhs, _Aarray, lda, _Carray, ldc, ref info, _devInfoArray, _Aarray.Length);
		...
		/// <param name="devInfoArray">null or optional array of integers of dimension batchsize.</param>
		/// <returns>0, if the parameters passed to the function are valid, &lt;0, if the parameter in postion -value is invalid</returns>
		public int GelsBatched(Operation trans, int m, int n, int nrhs, CudaDeviceVariable<cuFloatComplex>[] Aarray, int lda, CudaDeviceVariable<cuFloatComplex>[] Carray, int ldc, CudaDeviceVariable<int> devInfoArray)
		{
			int info = 0;
			CUdeviceptr[] _Aarray = new CUdeviceptr[Aarray.Length];
			CUdeviceptr[] _Carray = new CUdeviceptr[Carray.Length];
			...
			_status = CudaBlasNativeMethods.cublasCgelsBatched(_blasHandle, trans, m, n, nrhs, _Aarray, lda, _Carray, ldc, ref info, _devInfoArray, _Aarray.Length);
		...
		/// <param name="devInfoArray">null or optional array of integers of dimension batchsize.</param>
		/// <returns>0, if the parameters passed to the function are valid, &lt;0, if the parameter in postion -value is invalid</returns>
		public int GelsBatched(Operation trans, int m, int n, int nrhs, CudaDeviceVariable<cuDoubleComplex>[] Aarray, int lda, CudaDeviceVariable<cuDoubleComplex>[] Carray, int ldc, CudaDeviceVariable<int> devInfoArray)
		{
			int info = 0;
			CUdeviceptr[] _Aarray = new CUdeviceptr[Aarray.Length];
			CUdeviceptr[] _Carray = new CUdeviceptr[Carray.Length];
			...
			_status = CudaBlasNativeMethods.cublasZgelsBatched(_blasHandle, trans, m, n, nrhs, _Aarray, lda, _Carray, ldc, ref info, _devInfoArray, _Aarray.Length);
		...
		public int GetrsBatched(Operation trans, int n, int nrhs, CudaDeviceVariable<float>[] Aarray, int lda, CudaDeviceVariable<int> devIpiv,
			CudaDeviceVariable<float>[] Barray, int ldb, int batchSize)
		{
			if (Aarray.Length != batchSize || Aarray.Length != Barray.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasSgetrsBatched(_blasHandle, trans, n, nrhs, arrA, lda, devIpiv.DevicePointer, arrB, ldb, ref info, batchSize);
		...
		/// <param name="batchSize">number of pointers contained in A</param>
		/// <returns>If info=0, the execution is successful. If info = -j, the j-th parameter had an illegal value.</returns>
		public int GetrsBatched(Operation trans, int n, int nrhs, CudaDeviceVariable<double>[] Aarray, int lda, CudaDeviceVariable<int> devIpiv,
			CudaDeviceVariable<double>[] Barray, int ldb, int batchSize)
		{
			if (Aarray.Length != batchSize || Aarray.Length != Barray.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasDgetrsBatched(_blasHandle, trans, n, nrhs, arrA, lda, devIpiv.DevicePointer, arrB, ldb, ref info, batchSize);
		...
		/// <param name="batchSize">number of pointers contained in A</param>
		/// <returns>If info=0, the execution is successful. If info = -j, the j-th parameter had an illegal value.</returns>
		public int GetrsBatched(Operation trans, int n, int nrhs, CudaDeviceVariable<cuFloatComplex>[] Aarray, int lda, CudaDeviceVariable<int> devIpiv,
			CudaDeviceVariable<cuFloatComplex>[] Barray, int ldb, int batchSize)
		{
			if (Aarray.Length != batchSize || Aarray.Length != Barray.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasCgetrsBatched(_blasHandle, trans, n, nrhs, arrA, lda, devIpiv.DevicePointer, arrB, ldb, ref info, batchSize);
		...
		/// <param name="batchSize">number of pointers contained in A</param>
		/// <returns>If info=0, the execution is successful. If info = -j, the j-th parameter had an illegal value.</returns>
		public int GetrsBatched(Operation trans, int n, int nrhs, CudaDeviceVariable<cuDoubleComplex>[] Aarray, int lda, CudaDeviceVariable<int> devIpiv,
			CudaDeviceVariable<cuDoubleComplex>[] Barray, int ldb, int batchSize)
		{
			if (Aarray.Length != batchSize || Aarray.Length != Barray.Length)
				throw new CudaBlasException("Input array lengths are not equal to batchSize.");
			...
			_status = CudaBlasNativeMethods.cublasZgetrsBatched(_blasHandle, trans, n, nrhs, arrA, lda, devIpiv.DevicePointer, arrB, ldb, ref info, batchSize);
